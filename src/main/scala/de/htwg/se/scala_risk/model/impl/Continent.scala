...
case class Continent(name: String, countries: ArrayBuffer[TCountry],
  ...
  def toXml: scala.xml.Elem = {
    ...
    xml = XML.addXmlChild(xml, <bonusTroops>{ this.bonusTroops }</bonusTroops>).asInstanceOf[scala.xml.Elem]
    countries.foreach { x => xml = XML.addXmlChild(xml, <country>{ x.getName.toString() }</country>).asInstanceOf[scala.xml.Elem] }
    xml
  }
}