...
case class Country(name: String, world: TWorld, var neighboring_countries: Set[TCountry] = Set.empty,
  ...
  def toXml: scala.xml.Elem = {
    ...
    xml = XML.addXmlChild(xml, <name>{ this.name }</name>).asInstanceOf[scala.xml.Elem]
    xml = XML.addXmlChild(xml, <troops>{ this.troops }</troops>).asInstanceOf[scala.xml.Elem]
    xml = XML.addXmlChild(xml, <color>{ this.color }</color>).asInstanceOf[scala.xml.Elem]
    xml = XML.addXmlChild(xml, <owner>{ this.owner.getName.toString() }</owner>).asInstanceOf[scala.xml.Elem]
    var list = <list></list>
    this.neighboring_countries.foreach { x => list = XML.addXmlChild(list, <country>{ x.getName }</country>).asInstanceOf[scala.xml.Elem] }
    xml = XML.addXmlChild(xml, list).asInstanceOf[scala.xml.Elem]
    xml
  }
}