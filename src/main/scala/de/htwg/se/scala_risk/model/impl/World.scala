...
class World extends TWorld {
  ...
  class Countries(world: TWorld) {
    ...
    private def getCountriesXml(countries: ArrayBuffer[TCountry]): scala.xml.Elem = {
      var xml = <list></list>
      countries.foreach { x => xml = XML.addXmlChild(xml, <country>{ x.toXml }</country>).asInstanceOf[scala.xml.Elem] }
      xml
    }

    private def getContinentsXml(continents: ArrayBuffer[TContinent]): scala.xml.Elem = {
      var xml = <list></list>
      continents.foreach { x => xml = XML.addXmlChild(xml, <continent>{ x.toXml }</continent>).asInstanceOf[scala.xml.Elem] }
      xml
    }
  }
  /**
  ...
  class Players(world: TWorld) {
    ...
    private def getColorsXml(colors: List[Color]): scala.xml.Elem = {
      var xml = <list></list>
      this.colorList.foreach { x => xml = XML.addXmlChild(xml, <color>{ x.toString() }</color>).asInstanceOf[scala.xml.Elem] }
      xml
    }

    private def getPlayersXml(players: ArrayBuffer[TPlayer]): scala.xml.Elem = {
      var xml = <list></list>
      players.foreach { x => xml = XML.addXmlChild(xml, <player>{ x.toXml }</player>).asInstanceOf[scala.xml.Elem] }
      xml
    }
